...
// 导航打开监听事件
    ...
    public Cursor getAll(){ // 查詢所有資料
        ...
        Cursor c = db.rawQuery("SELECT _id, kind, item, quantity, limitdate, buyingdate, storage, unit,aid from icebox WHERE storage = '冷凍'   ORDER BY item", null);
        return c;
    }

    public Cursor getKind(String strkind){ // 查詢Kind
        Cursor c = db.rawQuery("SELECT _id, kind, item, quantity, limitdate, buyingdate, storage, unit,aid FROM icebox WHERE kind LIKE '%" + strkind + "%' AND storage = '冷凍'  ORDER BY item", null);
        return c;
    }
